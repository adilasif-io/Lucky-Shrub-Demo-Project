...
function compileHTML() {
  ...
  return src('src/pages/**/*.html')
    ...
    .pipe(
      replace(/<img[^>]+src="[^"]+\.(png|jpg|jpeg)"/g, (match) => {
        // Replace image extensions in the src attribute.
...
function jsVendor() {
  ...
  return src([
      ...
      'vendors/bootstrapv5-multiselect/js/bootstrap-multiselect.js',
...
function generateExcel() {
  ...
  iOS_workspace.protect('adilasif-io', options);
  android_workspace.protect('adilasif-io', options);
  net_workspace.protect('adilasif-io', options);
...
function deploy(cb) {
  const argv = yargs.options({
    remotePath: { type: 'string', demandOption: true },
  }).argv;
  ...
  //TODO: Move to env
    const conn = ftp.create({
      host: 'host@hostname',
      port:21,
      user: 'admin@hostname.com',
      password: 'PASSWORD',
      log: console.log
    });
  log(chalk.yellow.bold('***Connecting to FTP server...***'));
  
    const stream = src(['dist/**/*'], { buffer: false })
      .pipe(conn.dest(argv.remotePath)); // Set the remotePath to '/'
  
    stream.on('end', function() {
      log(chalk.yellow.bold('***File upload complete!***'));
